...
class FSRSScheduler {
  ...
  FSRSScheduler({double desiredRetention = defaultDesiredRetention})
      : desiredRetention =
            desiredRetention.clamp(minDesiredRetention, maxDesiredRetention);
  ...
  Map<String, double> calculateNextInterval({
    ...
    double retrievability =
        _calculateRetrievability(currentStability, elapsedDays);
  ...
  double _updateDifficulty(double currentDifficulty, int grade) {
    ...
    double newDifficulty =
        currentDifficulty - gradeWeight + _w1 * (currentDifficulty - 0.5);
  ...
  double _updateStability(
    ...
    if (grade == 0) {
      ...
      newStability =
          currentStability * (1 + _w5 * difficultyFactor * retentionFactor);
  ...
  double _calculateOptimalInterval(double stability) {
    ...
    double interval =
        stability * (pow(1 / desiredRetention, 1 / _w2).toDouble() - 1);
  ...
  Map<String, double> _calculateGradients() {
    ...
    // Get the last 500 reviews (or all if less than 500)
    final recentHistory = _performanceHistory.length > 500
        ? _performanceHistory.sublist(_performanceHistory.length - 500)
        : _performanceHistory;

    for (var review in recentHistory) {
  ...
  Map<String, dynamic> getStats() {
    ...
    if (_performanceHistory.isNotEmpty) {
      double sum = _performanceHistory.fold(0.0, (sum, review) {
        return sum +
            _calculateRetrievability(
                review['stability'], review['interval'].toDouble());
      });
  ...
  void importState(Map<String, dynamic> state) {
    ...
    if (state.containsKey('performanceHistory')) {
      _performanceHistory.clear();
      _performanceHistory
          .addAll(List<Map<String, dynamic>>.from(state['performanceHistory']));
    }
  }
}
